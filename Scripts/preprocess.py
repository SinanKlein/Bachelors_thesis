...
def stats_to_long(stats: dict, source: str, feature_names=None, n_rows: int | None = None) -> pd.DataFrame:
    df = pd.DataFrame({
        "source":     source,
    ...
    # Raw total abundance/count across entity rows. Useful for descriptive ProC plots.
    n_rows = int(n_rows) if n_rows is not None else 1
    df["total_abundance"] = df["mean"] * n_rows
...
def resolve_quantile(quantile_cfg, source: str, default: float = 0.10) -> float:
    """
    `quantile` in the config may be either a single number (applied to every
    source) or a mapping {source: q}. This returns the right value for `source`.
    """
...
def main():
    ...
    print("[preprocess] === STEP 2/6: raw-variance filtering, then CLR transform ===")
    pcfg = cfg["preprocess"]
    ...
    for source in pcfg["apply_to"]:
        ...
        print(f"[preprocess] {source}: input shape {X_raw.shape} | keep top {100*q:.0f}% by RAW variance")
        ...
        X_raw_filt = reducer.fit_transform(X_raw)
        X_clr_filt = clr_transform(X_raw_filt, pseudocount=pseudocount)
        print(f"[preprocess] {source}: kept {X_clr_filt.shape[1]}/{X_raw.shape[1]} features "
              f"(raw-var threshold = {reducer.feature_stats()['threshold']:.4g}; CLR pseudocount = {pseudocount:g})")

        filtered_arrays[f"{source}_filtered"] = X_clr_filt
        filtered_arrays[f"{source}_raw_filtered"] = X_raw_filt.astype(np.float32)
        ...
        feature_stats_long.append(stats_to_long(reducer.feature_stats(), source, feature_names, n_rows=X_raw.shape[0]))
