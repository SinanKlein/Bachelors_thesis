# ==============================================================================
# Publication-Ready Plotting Script for VIB / MLP Experiments
# ==============================================================================
library(dplyr)
library(tidyr)
library(ggplot2)
...
# ---- Load Data ----
df <- read.csv(file.path(data_dir, "metrics_all_splits.csv")) %>% clean_names()
...
df_lat_long <- df_lat %>%
  ...
  mutate(
    model = toupper(model),
    metric = ifelse(metric == "auc_y", "Task Performance (AUC Y)", "Confounder Prediction (AUC W0)")
  ) %>%
...
# ==============================================================================
# 4. Info Theory Trace Plots (VIB) - Grouped/Aggregated Over Splits
# ==============================================================================
...
# C2. Dual Variable Dynamics (Beta)
...
# C3. Validation Performance Over Training
...
# ==============================================================================
# 6. Rate-Distortion Trade-off (Claim 2)
# ==============================================================================
...
# ==============================================================================
# 7. Multi-Task Distortion Geometry (Claim 5)
# ==============================================================================
# We plot the joint distortion plane: Error on Y vs Error on W
df_plane <- df_lat %>%
...
p_plane <- ggplot(df_plane, aes(x = mean_dist_y, y = mean_dist_w, color = model, shape = model)) +
  ...
  labs(title = "Multi-Task Distortion Plane", 
       ...
       y = "Confounder Task Distortion (RMSE W)") +
  theme_pub + color_palette

ggsave(file.path(out_dir, "distortion_plane.png"), p_plane, width = 8, height = 6, dpi = 300)

# ==============================================================================
# 8. Efficiency of Information Allocation (Claim 8)
# ==============================================================================
