...
plot_mean_var_single <- function(df_one, source_name, accent_color) {
  ...
  composite <- p_top + plot_spacer() +
    ...
    plot_annotation(
      ...
      subtitle = sprintf("%s features total | %s kept (top %.0f%% by raw variance) | threshold = %.3g",
...
p3 <- ggplot(fs, aes(x = mean, y = var, color = selected)) +
  ...
  labs(
    ...
    subtitle = "Pseudo-log axes. Color = top-quantile selection by raw variance score.",
...
# Uses feature_name and total_abundance written by preprocess.py. This gives a
# direct view of which ProC/protein-cluster columns dominate before CLR.
if (all(c("feature_name", "total_abundance") %in% names(fs))) {
  ...
  p6b <- ggplot(top_pc, aes(x = total_abundance, y = label, fill = source)) +
    ...
    labs(
      ...
      subtitle = "Top 25 per source before CLR transformation",
