...
# ----------------------------
#   * Edge scope = all pairs in Y (now intersected with feature availability).
#   * Variance based feature selection (same quantile mechanism, same default).
#   * 10 stratified 80/10/10 splits (StratifiedKFold on the binary Y).
#   * W aligned onto the Y grid, fill 0, with a W observation mask.
#   * Output keys in split_k*.npz and graph_data.npz are byte-for-byte the same
#     names the downstream pipeline (preprocess.py) reads.
#
...
# CONFIG 
# ===========================================================================
BASE = Path(r"C:\Sinan_Klein\LMU\lmu_thesis\datas_final\SINAN_datasets\data\IBD")
...
#  Y : CRISPR linkage tables (paths are folder/inner-csv, as provided) 
Y_FILES = [
    BASE / "abundance_derived" / "network" / "crispr_genus_by_votu_binary.csv" / "crispr_genus_by_votu_binary.csv",
    BASE / "abundance_derived" / "network" / "crispr_genus_by_votu_counts.csv" / "crispr_genus_by_votu_counts.csv",
    BASE / "abundance_derived" / "network" / "crispr_host_taxid_by_votu_binary.csv" / "crispr_host_taxid_by_votu_binary.csv",
    BASE / "abundance_derived" / "network" / "crispr_host_taxid_by_votu_counts.csv" / "crispr_host_taxid_by_votu_counts.csv",
]
...
W_FILES = [
    BASE / "Networks" / "Bipartite_edge_probability" / "glasso"
         / "IBD_glasso_edge_probability_bacteria_virus_stars01.csv"
         / "IBD_glasso_edge_probability_bacteria_virus_stars01.csv",
    BASE / "Networks" / "Bipartite_edge_probability" / "glasso"
         / "IBD_glasso_edge_probability_bacteria_virus_stars02.csv"
         / "IBD_glasso_edge_probability_bacteria_virus_stars02.csv",
]
ACTIVE_W = 0          # 0 = stars01
