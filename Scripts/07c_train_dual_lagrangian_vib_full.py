...
#   1) VIB is now STRICT single channel:
#        - one latent z
#        - one KL
#        - one beta
#        - one budget C
#        - all heads consume the same z (full sharing)
#   2) Outer loop over latent_dim = 1..6 (runs EVERYTHING for each latent_dim)
#   3) Stores info-theoretic metrics (rate/distortion style traces) to a separate CSV in OUTPUT_DIR
#
# Everything else (baselines, losses, early stopping, hyperparams, paths, splits, etc.) stays the same style.
...
# We will also write:
#   - metrics_all_latdims.csv
#   - metrics_summary_latdims.csv
#   - info_theory_trace_latdims.csv


# ------------------------------------------------------------------
# 0. Config (UNCHANGED values)
# ------------------------------------------------------------------
config = {
    # Original baseline parameters
    "latent_dim": 3,        # (will be overridden by loop d=1..6, but value unchanged here)
    "hidden_dim": 64,
    "dropout": 0.3,
    "head_hidden_dim": 32,  # (unused in your current heads, kept unchanged)
    "lambda_y": 1.0,
    "lambda_w": 1.0,
    "lambda_s": 2.0,
    "use_W_tasks": True,
    ...
    # VIB parameters (kept unchanged; some are reused in strict single form)
    "latent_dim_Y": 2,       # kept unchanged but NOT used in strict single channel
    "latent_dim_W": 1,       # kept unchanged but NOT used in strict single channel
    "warmup_epochs": 50,
    "dual_lr": 1e-3,
    "beta_init": 1e-4,
    "beta_max": 10.0,
    "mY": 0.7,               # used as the single budget multiplier m (strict single)
    "mW": 0.9,               # kept unchanged but NOT used in strict single channel
    "C_min": 0.05,
    "use_moving_avg": True,
    "KL_smoothing": 0.99
}
...
def run_one_split(split_idx: int, latent_dim: int):
    ...
    for epoch in range(1, config["n_epochs"] + 1):
        ...
        for Xb, Yb, Wb, Sb in train_loader:
            ...
            KL_batch = kl_diag_gaussian(mu, logvar) / float(latent_dim)  # per-dim "rate"
            ...
            if epoch <= config["warmup_epochs"]:
                ...
                if epoch > config["warmup_epochs"] - 4:
            ...
            # PHASE 2: CONSTRAINED DUAL ASCENT
                ...
                if epoch == config["warmup_epochs"] + 1 and C == config["C_min"]:
                    ...
                    # Strict single uses mY as the multiplier m (kept unchanged)
                    C = max(config["C_min"], config["mY"] * KL_med)
                    KL_ma = KL_med
    ...
    # ================================================================
    # EXTRA METRICS (ADDED): train/test eval loss, generalization gap,
...
# ------------------------------------------------------------------
# Main: loop over latent_dim=1..6 and over splits
# ------------------------------------------------------------------
if __name__ == "__main__":
    all_metrics = []
    all_info_rows = []

    latent_dims_to_run = list(range(1, 11))  # 1..6
